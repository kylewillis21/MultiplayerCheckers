...
void game::stringToGame(std::string s){
    if (s.length() < 65){
        cout << "Error: String not long enough in game::stringToGame()" << endl;
        return;
    }

    // Index 0-63 will be the board
    gameBoard.stringToBoard(s.substr(0,64));

    // Index 64+ will be the turn number
    int num = stoi(s.substr(64,3));
    setTurnNum(num);
}

std::string game::gameToString(){
    std::string board = gameBoard.boardToString();
    return board += std::to_string(getTurnNum());
}
